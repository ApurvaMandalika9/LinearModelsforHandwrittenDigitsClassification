...
def prepare_X(raw_X):
    ...
    Fsymmetry = -np.sum(np.abs(raw_image - np.flip(raw_image, axis=2)), axis=(1, 2)) / 256

    # Feature 2: Measure of Intensity
    Fintensity = np.sum(raw_image, axis=(1, 2)) / 256
    ...
    X = np.hstack([bias_term, Fsymmetry.reshape(-1, 1), Fintensity.reshape(-1, 1)])
